...
if __name__ == "__main__":
    ...
    expert_trajs = torch.load(
        f'expert_data/states/{env_name}.pt').numpy()[:, :, state_indices]
    expert_trajs = expert_trajs[:
                                num_expert_trajs, :, :]  # select first expert_episodes
    ...
    for itr in range(v['irl']['n_itrs']):
        ...
        reward_losses = []
        expert_res_indices = np.random.choice(expert_trajs.shape[0],
                                              v['irl']['resample_episodes'],
                                              replace=True)
        expert_trajs_train = expert_trajs[expert_res_indices].copy(
        )  # resampling the expert trajectories

        # Compute sample gradient (Eq. 3)
        loss, _ = f_div_disc_loss(v['obj'],
                                  v['IS'],
                                  samples,
                                  disc,
                                  reward_func,
