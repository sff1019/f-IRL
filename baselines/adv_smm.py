...
class AdvSMM:
    ...
    def __init__(
        ...
        self.mode = obj
        assert self.mode in ['f-max-rkl', 'gail', 'fairl',
                             'airl'], 'Invalid adversarial irl algorithm!'
        if self.mode == 'airl':
        ...
        self.state_indices = torch.LongTensor(state_indices).to(
            device)  # for Disciminator observation space
        self.rho_expert = rho_expert
        self.target_state_buffer = self.process_target_state_buffer(
            target_state_buffer, expert_action_trajs)
        self.agent = agent
        ...
        for epoch in range(1, self.num_epochs + 1):
            for steps_this_epoch in range(self.num_steps_per_epoch):
                a = self.agent.get_action(
                    o)  # now assumes no early random exploration

                o2, _, d, _ = self.env.step(
                    a)  # reward is set by discrimator, not here
    ...
    def process_target_state_buffer(self, target_state_buffer,
                                    expert_action_trajs):
    ...
    def _do_reward_training(self, epoch):
        ...
        if self.expert_IS:  # assume we can only use policy states
            ...
            policy_disc_input = self.replay_buffer.sample_batch(
                self.disc_optim_batch_size)['obs']

            # fit an agent density, using all samples in replay buffer
            agent_density = neighbors.KernelDensity(bandwidth=self.bandwidth,
                                                    kernel=self.kernel)
            agent_samples = self.replay_buffer.state[:,
                                                     self.state_indices.cpu().
                                                     numpy().astype(np.uint8)]
            agent_density.fit(agent_samples)

            # get the importance sampling weight for the expert stat
            policy_input = torch.index_select(
                policy_disc_input, 1, self.state_indices).cpu().numpy()
            agent_sample_density = np.exp(
                agent_density.score_samples(policy_input))
            ...
            sampled_batch = self.replay_buffer.sample_batch(
            ...
            disc_logits, _ = self.disc_forward_airl(
                policy_state, policy_action, policy_next_state, expert_state,
            ...
            if self.use_grad_pen:  # gradient penalty on reward and value separately
                # grad pen on reward
                eps = torch.rand(
                    (self.disc_optim_batch_size, 1)).to(self.device)
                interp_obs = eps * expert_state + (
                    1 - eps) * policy_state  # interpolate
                ...
                eps2 = torch.rand(
                    (self.disc_optim_batch_size, 1)).to(self.device)
                interp_obs2 = eps2 * expert_state + (
                    1 - eps2) * policy_state  # use either cur or next state
    ...
    def _do_policy_training(self, epoch):
        policy_batch = self.replay_buffer.sample_batch(
            self.policy_optim_batch_size)
        obs, obs2 = policy_batch['obs'], policy_batch['obs2']
        policy_batch['rew'] = self.get_reward(obs, obs2)

        # policy optimization step
        agent_stat = self.agent.update(
            policy_batch)  # loss-q, loss-pi, log-pi.
        # in original f-MAX, it adds regularization on policy mean & logstd

        reward_stat = np.array([
            policy_batch['rew'].mean().item(),
            policy_batch['rew'].std().item(), policy_batch['rew'].max().item(),
            policy_batch['rew'].min().item()
        ])
    ...
    def disc_forward(self, policy_disc_input):
        ...
        if policy_disc_input.shape[1] > len(self.state_indices):
            disc_input = torch.index_select(policy_disc_input, 1,
                                            self.state_indices)
        else:
    ...
    def get_reward(self, obs, obs2=None):
        if self.mode != 'airl':
            ...
            with torch.no_grad():
                disc_logits, _ = self.disc_forward(
                    obs)  # D' = log(D) - log(1-D)
            ...
            if self.mode in ['f-max-rkl']:
                ...
                rewards = F.softplus(
                    disc_logits,
                    beta=-1)  # -log(1-D) = -log(1+e^-D') ignore constant log2
            elif self.mode == 'fairl':
                rewards = torch.exp(disc_logits) * (-1.0 * disc_logits
                                                    )  # exp(D')*(-D')
        else:
            ...
            with torch.no_grad():
                ...
                if self.airl_rew_shaping:
                    rewards += self.gamma * self.value_model(
                        obs2) - self.value_model(obs)
    ...
    def record_discrim(self, disc_stat):
        ...
        self.logger.record_tabular("disc_gp",
                                   round(disc_stat[0] - disc_stat[1], 4))
